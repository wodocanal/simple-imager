    ...
    private static func openExpectedDisk(
        ...
        repeat {
            ...
            if let permissionFailure {
                throw AppError.commandFailed(
                    "macOS заблокировала прямой доступ к накопителю. Включите «Полный доступ к диску» для приложения «SD Архиватор» в разделе «Конфиденциальность и безопасность», затем полностью перезапустите приложение. \(permissionFailure)"
                )
            }
